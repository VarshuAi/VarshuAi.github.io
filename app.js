...
if (window.matchMedia('(pointer: fine)').matches) {
  let tY = window.scrollY, cY = window.scrollY, running = false;
  window.addEventListener('wheel', e => {
    tY = Math.max(0, Math.min(document.documentElement.scrollHeight - window.innerHeight, tY + e.deltaY * 1.1));
    if (!running) { running = true; step(); }
  }, { passive: true });
  function step() {
    cY += (tY - cY) * 0.09;
    window.scrollTo(0, cY);
    if (Math.abs(tY - cY) > 0.4) requestAnimationFrame(step);
    else { cY = tY; running = false; }
  }
}
